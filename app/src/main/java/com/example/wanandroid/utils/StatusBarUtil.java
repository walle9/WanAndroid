...
//        window.setStatusBarColor(Color.TRANSPARENT);
     ...
     * 设置状态栏及导航栏
     *
     * @param activity                   需要修改的activity
     * @param isImmersive                是否沉浸式,andriud9和锁屏都有问题,慎用
     * @param isModifyStatusBar          是否修改状态栏颜色   (false时,下一个属性无效)
     * @param isStatusBarTransparent     修改的状态栏颜色是否为全透明  true:全透明,false:半透明
     * @param isModifyNavigationBar      是否修改导航栏颜色   (false时,下一个属性无效)
     * @param isNavigationBarTransparent 修改的导航栏颜色是否为全透明  true:全透明,false:半透明
     */
    public static void setBar(Activity activity, boolean isImmersive, boolean isModifyStatusBar, boolean isStatusBarTransparent, boolean isModifyNavigationBar, boolean isNavigationBarTransparent) {

        Window window = activity.getWindow();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {//5.0及以上
            View decorView = window.getDecorView();                 //获取顶级窗口
            int systemUiVisibility = 0;
            if (isImmersive) {
                systemUiVisibility |= View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY                      //粘性沉浸体验
                        | View.SYSTEM_UI_FLAG_HIDE_NAVIGATION                                   //自动隐藏导航
                        | View.SYSTEM_UI_FLAG_FULLSCREEN;                                       //自动隐藏状态栏
            }

            if (isModifyStatusBar) {
                systemUiVisibility |=View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN                      //使状态栏出现的时候，不会重新调整activity的高度，状态栏覆盖在activity之上
                |View.SYSTEM_UI_FLAG_LAYOUT_STABLE;                                             //让应用的主体内容占用系统状态栏的空间
                if (isStatusBarTransparent) {
                    window.setStatusBarColor(Color.TRANSPARENT);                                //设置状态栏颜色透明

                } else {
                    window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);        //设置状态栏为半透明
                }
            }

            if (isModifyNavigationBar) {
                systemUiVisibility|=View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION;                 //标志布局会侵入到导航栏下
                if (isNavigationBarTransparent) {
                    window.setNavigationBarColor(Color.TRANSPARENT);                            //设置导航栏颜色透明
                } else {
                    window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_NAVIGATION);    //设置导航栏为半透明
                }
            }

            if (systemUiVisibility!=0) {

                decorView.setSystemUiVisibility(systemUiVisibility);                //设置系统UI属性
            }

            if (isModifyStatusBar&&isStatusBarTransparent) {
